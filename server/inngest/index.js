...
const sendBookingConfirmationEmail = inngest.createFunction(
  ...
  const sendShowReminders = inngest.createFunction(
    ...
    async ({ step }) => {
      ...
      const windowStart = new Date(in8Hours.getTime() - 10 * 60 * 1000); // 10 minute window
  
      // Prepare reminder tasks
      const reminderTasks = await step.run("prepare-reminder-taks", async () => {
        ...
        for (const show of shows) {
          ...
          // Get unique user IDs from occupied seats
